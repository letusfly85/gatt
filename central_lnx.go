...
func (c *central) loop() {
	for {
		n, err := c.l2conn.Read(c.rxBuf)
		if n == 0 || err != nil {
			c.Close()
			break
		}
		if rsp := c.handleReq(c.rxBuf[:n]); rsp != nil {
			c.l2conn.Write(rsp)
		}
	}
}
...
func (c *central) handleMTU(r ExchangeMTURequest) []byte {
	...
	c.l2conn.SetTxMTU(int(txMTU))
	c.l2conn.SetRxMTU(len(c.rxBuf))

	c.txBuf = make([]byte, txMTU)
	rsp := ExchangeMTUResponse(c.txBuf)
	rsp.SetAttributeOpcode()
	rsp.SetServerRxMTU(uint16(len(c.rxBuf)))
	return rsp[:3]
}

func (c *central) handleFindInfo(r FindInformationRequest) []byte {
	rsp := FindInformationResponse(c.txBuf)
	rsp.SetAttributeOpcode()
	rsp.SetFormat(0x00)
...
func (c *central) handleFindByTypeValue(r FindByTypeValueRequest) []byte {
	...
	rsp := FindByTypeValueResponse(c.txBuf)
	rsp.SetAttributeOpcode()
...
func (c *central) handleReadByType(r ReadByTypeRequest) []byte {
	rsp := ReadByTypeResponse(c.txBuf)
	rsp.SetAttributeOpcode()
	...
	for _, a := range c.attrs.Subrange(r.StartingHandle(), r.EndingHandle()) {
		...
		v := a.value
		if v == nil {
			rsp := newResponseWriter(len(c.txBuf) - 1)
			req := &Request{
				Central: c,
				Cap:     len(c.txBuf) - 1,
				Offset:  0,
...
func (c *central) handleReadByGroup(r ReadByGroupTypeRequest) []byte {
	...
	rsp := ReadByGroupTypeResponse(c.txBuf)
	rsp.SetAttributeOpcode()
	buf := bytes.NewBuffer(rsp.AttributeDataList())
	buf.Reset()
...
func (c *central) sendNotification(a *attr, data []byte) (int, error) {
	rsp := HandleValueNotification(c.txBuf)
	rsp.SetAttributeOpcode()
